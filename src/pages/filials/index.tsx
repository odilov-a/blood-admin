...
const Filial = () => {
  ...
  const onDeleteHandler = (id: string) => {
    Modal.confirm({
      title: t("Вы действительно хотите удалить filials?"),
      okText: t("да"),
      okType: "danger",
      cancelText: t("нет"),
  ...
  const deleteAction = (id: string) => {
    if (id) {
      mutate(
        { method: "delete", url: `/filials/${id}`, data: null },
        {
          onSuccess: () => {
  ...
  return (
    <div className="flex">
      ...
      <div>
        <Container.All name="filials" url="/filials">
          {({ items, isLoading }) => {
            return (
              <div>
                <Button
                        ...
                        <Col className="gutter-row mb-5" span={6}>
                          ...
                          >
                            <Meta
                              ...
                              title={
                                <div className="mb-1">
                                  <p className="dark:text-[#e5e7eb] block truncate">{(get(card, "location", ""))}</p>
                                  <p className="dark:text-[#e5e7eb] block truncate">{(get(card, "filialName", ""))}</p>
                                </div>
                              }
                              description={
                                <div className="flex justify-between items-center mb-2">
                                  <p className="dark:text-[#e5e7eb] line-clamp-3">{(get(card, "phoneNumber", ""))}</p>
                                  <p className="dark:text-[#e5e7eb] line-clamp-3">{(get(card, "workingTime", ""))}</p>
                                </div>
                              }
                            />
